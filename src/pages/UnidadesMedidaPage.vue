<template>
  <q-page class="q-pa-md">
    <div class="row items-center q-mb-md">
      <div class="text-h6">{{ t('titulos.unidades') }}</div>
      <q-space />
      ...
      <q-card style="min-width: 340px">
        ...
        <q-card-section>
          <q-form class="q-gutter-md" @submit="salvar">
            ...
            <q-input v-model="form.sigla" label="Sigla" :rules="[(v) => !!v || 'Informe sigla']" />
            <q-select
              v-model="form.tipo"
              :options="['UNIDADE', 'PESO', 'VOLUME', 'COMPRIMENTO']"
              label="Tipo"
            />
            <q-input v-model.number="form.fatorParaBase" type="number" label="Fator para base" />
...
import type { UnidadeMedida, UnidadeMedidaRequest } from 'src/services/unidadesMedida'
...
const form = reactive<UnidadeMedidaRequest>({
  nome: '',
  sigla: '',
  tipo: 'UNIDADE',
  fatorParaBase: null
...
function abrirNovo() {
  editando.value = null
  Object.assign(form, { nome: '', sigla: '', tipo: 'UNIDADE', fatorParaBase: null })
  dialog.value = true
}

function abrirEdicao(row: UnidadeMedida) {
  editando.value = row.id
  Object.assign(form, {
    nome: row.nome,
    sigla: row.sigla,
    tipo: row.tipo,
    fatorParaBase: row.fatorParaBase
  })
  dialog.value = true
}
...
function confirmarRemocao(row: UnidadeMedida) {
