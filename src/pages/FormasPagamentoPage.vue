<template>
  <q-page class="q-pa-md">
    <div class="row items-center q-mb-md">
      <div class="text-h6">{{ t('titulos.formasPagamento') }}</div>
      <q-space />
      ...
      <q-card style="min-width: 340px">
        ...
        <q-card-section>
          <q-form class="q-gutter-md" @submit="salvar">
            ...
            <q-select
              v-model="form.tipo"
              :options="['DINHEIRO', 'PIX', 'DEBITO', 'CREDITO', 'BOLETO', 'TRANSFERENCIA']"
              label="Tipo"
            />
            <q-input v-model.number="form.diaFechamento" type="number" label="Dia fechamento" />
            <q-input v-model.number="form.diaVencimento" type="number" label="Dia vencimento" />
...
import { useFormaPagamentoStore } from 'stores/formasPagamento'
import type { FormaPagamento, FormaPagamentoRequest } from 'src/services/formasPagamento'
...
const form = reactive<FormaPagamentoRequest>({
  nome: '',
  tipo: 'DINHEIRO',
  diaFechamento: null,
  diaVencimento: null
...
function abrirNovo() {
  editando.value = null
  Object.assign(form, { nome: '', tipo: 'DINHEIRO', diaFechamento: null, diaVencimento: null })
  dialog.value = true
}

function abrirEdicao(row: FormaPagamento) {
...
function confirmarRemocao(row: FormaPagamento) {
